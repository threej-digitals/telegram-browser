...
async function scrapChat(username) {
  ...
  var chatDetails = scrapChatDetailsAndViews(
    res.data,
    username,
    chatDetailsFormat
  );
...
function scrapChatDetailsAndViews(html, username, chatDetailsFormat) {
  try {
    ...
    //store postcount
    chatDetails.POSTCOUNT = $(
      "body > main > div > section > div:last-child > div"
    )
      .attr("data-post")
      .split("/")[1]++;
    let views = 0;
    ...
    for (i; i <= 10; i++) {
      let t = $(
        "body > main > div > section > div:nth-child(" + (length - i) + ")"
      )
        .find(
          "div.tgme_widget_message.text_not_supported_wrap.js-widget_message > div.tgme_widget_message_bubble > div.tgme_widget_message_footer.compact.js-message_footer > div > span.tgme_widget_message_views"
        )
        .text();
      if (t == "") break;
    ...
    chatDetails.TITLE =
      $(
        "body > header > div > div.tgme_header_right_column > section > div > div.tgme_channel_info_header > div.tgme_channel_info_header_title_wrap > div.tgme_channel_info_header_title > span"
      ).text() || "";

    //description
    chatDetails.DESCRIPTION =
      $(
        "body > header > div > div.tgme_header_right_column > section > div > div.tgme_channel_info_description"
      ).html() || "";
    ...
    //chat counters
    $(
      "body > header > div > div.tgme_header_right_column > section > div > div.tgme_channel_info_counters > div"
    ).each((k, v) => {
      const type = $(v).find("span.counter_type").text();
      const value = $(v).find("span.counter_value").text();
      const key = /photos?/i.test(type)
        ? "PICSCOUNT"
        : /videos?/i.test(type)
        ? "VIDEOSCOUNT"
        : /files?/i.test(type)
