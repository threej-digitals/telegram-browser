...
export default async function getChat(category, language) {
  const chats = await threej.query(
    "SELECT CID, USERNAME, POSTCOUNT, CUPDATE, TITLE, DESCRIPTION, PHOTO, SUBSCOUNT, PICSCOUNT, VIDEOSCOUNT, LINKSCOUNT, FILECOUNT FROM ?? WHERE CATEGORY LIKE ? AND CLANGUAGE LIKE ? AND USERNAME IS NOT NULL AND CTYPE = 'channel' ORDER BY SUBSCOUNT DESC LIMIT 5",
