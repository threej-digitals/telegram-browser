...
class Threej {
  ...
  async base64ToImg(base64String, fileLocation) {
    if (typeof base64String != "string" || typeof fileLocation != "string")
  ...
  parseCSV(str, delimiter = ",") {
    if (typeof str != "string")
      throw new Error("CSV string expected received " + typeof str);
  ...
  async saveRemoteFile(url, storagePath, filename = "F" + Date.now()) {
    try {
      const writer = fs.createWriteStream(
        path.resolve(storagePath, filename + path.extname(url))
      );
      ...
      return new Promise((resolve, reject) => {
        writer.on(
          "finish",
          resolve(storagePath + filename + path.extname(url))
        );
